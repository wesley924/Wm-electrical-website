...
function authHeaders() {
  ...
  const email = process.env.SM8_ACCOUNT_EMAIL; // the ServiceM8 login email tied to the API key
  if (!apiKey || !email) {
    throw new Error("Missing SM8_API_KEY or SM8_ACCOUNT_EMAIL environment variables");
  }
...
async function getFormResponsesForJob(jobUuid) {
  const filter = encodeURIComponent(`job_uuid eq '${jobUuid}'`);
...
async function getAttachmentUrl(attachmentUuid) {
  ...
  // ServiceM8 returns a `photo_url`/file reference for image attachments;
  // confirm the exact field name against your account's API response the first time you run this.
  return attachment.photo_url || attachment.file_url || null;
}
